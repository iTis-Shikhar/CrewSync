      ...
      <form onSubmit={handleSubmit} className="auth-form">
        ...
        {!isLogin && (
          <>
            ...
            {/* NEW: Skills field, only for volunteers */}
            {role === 'volunteer' && (
              <div className="form-group">
                <label htmlFor="skills">Skills/Interests (comma separated)</label>
                <input type="text" id="skills" value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="e.g., First Aid, Photography, Public Speaking" />
              </div>
            )}
          </>
        )}
        ...
        <p className="toggle-form">
          ...
          <span onClick={() => setIsLogin(!isLogin)} className="toggle-link">{isLogin ? ' Register' : ' Login'}</span>
        </p>
      </form>
    </div>
  );
